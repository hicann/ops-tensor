...
public:
    ...
    __aicore__ inline void Run()
    {
        ...
        for (uint64_t index = 0; index < aivMte2Num_; ++index) {
            ...
            if constexpr (sizeof(OutType) == sizeof(half)) {
                ...
                if constexpr (
                    DispatchPolicy::FUSED_OP_TYPE == OP_TYPE_RELU && !AscendC::IsSameType<OutType, bfloat16_t>::value) {
                ...
                AscendC::DataCopyPad<
                    OutType, (DispatchPolicy::FIXP_OPTI == MatMulL0C2Out::ND_FIXPIPE_1_2) ? AscendC::PaddingMode::Normal :
    ...
    __aicore__ inline void UpdateAivBasicBlock()
    {
        if (round_ < NUM_TWO) {
            ...
            if constexpr (DispatchPolicy::FIXP_OPTI == MatMulL0C2Out::ND_FIXPIPE_1_2) {
