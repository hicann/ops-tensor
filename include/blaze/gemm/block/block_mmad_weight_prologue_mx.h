...
class BlockMmad<
    MatmulWithWeightQuantMx, ATypeTuple_, LayoutATuple_, BTypeTuple_, LayoutBTuple_, CType_, LayoutC_, BiasType_,
    LayoutBias_> {
public:
    ...
    static_assert(
        AscendC::Std::is_same_v<ScaleAType, AscendC::fp8_e8m0_t> &&
    ...
    __aicore__ inline void operator()(
        const TensorA_& tensorA, const TensorScaleA_& tensorScaleA, const TensorScaleB_& tensorScaleB,
        const TensorC_& tensorC)
    {
    ...
    public:
        __aicore__ inline void Init(uint64_t baseN, uint64_t kL1Size, uint64_t bufferNum, bool hasBias)
        {
            ...
            for (uint16_t bufferId = 0; bufferId < bufferNum_; ++bufferId) {
                ...
                biasOffsets_[bufferId] =
                    halfOffset + buffersPerHalf_ * weightBufferSize_ + slotInHalf * biasBufferSize_;
            }
        }

        __aicore__ inline uint64_t WeightBufferSize() const
        {
            return weightBufferSize_;
        }

        __aicore__ inline uint64_t BiasBufferSize() const
        {
            return biasBufferSize_;
        }

        __aicore__ inline uint64_t BuffersPerHalf() const
        {
            return buffersPerHalf_;
        }

        __aicore__ inline auto MakeWeightTensor(
            uint64_t bufferId, uint64_t nOffset, uint64_t kOffset, uint64_t nStride, int64_t nSize, int64_t kSize) const
        {
            auto fullLayout = AscendC::Te::MakeFrameLayout<
                AscendC::Te::ZNLayoutPtn, AscendC::Std::Int<AscendC::Te::C0_ELEMENT<ConvertedBType>>>(
...
private:
    ...
    public:
        __aicore__ inline void Init(
            uint64_t baseM, uint64_t baseN, uint64_t kL1Size, uint64_t scaleKL1Size, uint64_t bufferNum, bool hasBias)
        {
            ...
            for (uint16_t bufferId = 0; bufferId < bufferNum; ++bufferId) {
                ...
                aOffsets_[bufferId] =
                    halfOffset +
                    buffersPerHalf * (weightStorage_.WeightBufferSize() + weightStorage_.BiasBufferSize()) +
                    slotInHalf * aBufferSize;
            }
            for (uint16_t bufferId = 0; bufferId < DOUBLE_BUFFER_COUNT; ++bufferId) {
                uint64_t halfOffset = (bufferId & 1U) * HALF_L1_SIZE;
                scaleBOffsets_[bufferId] =
                    halfOffset + buffersPerHalf * (weightStorage_.WeightBufferSize() + weightStorage_.BiasBufferSize() +
                                                   aBufferSize);
        ...
        {
            auto layout = AscendC::Te::MakeFrameLayout<
                AscendC::Te::NZLayoutPtn, AscendC::Std::Int<AscendC::Te::C0_ELEMENT<AType>>>(mSize, kSize);
            return AscendC::Te::MakeTensor(
        ...
        __aicore__ inline uint64_t AOffset(uint64_t bufferId) const
        {
            return aOffsets_[bufferId];
        }

        __aicore__ inline uint64_t ScaleAOffset(uint64_t bufferId) const
        {
            return scaleAOffsets_[bufferId];
        }

        __aicore__ inline uint64_t ScaleBOffset(uint64_t bufferId) const
        {
            return scaleBOffsets_[bufferId];
        }
    ...
    {
        uint64_t l1BaseM = static_cast<uint64_t>(AscendC::Te::Get<0>(params.l1TileShape));
        uint64_t l1BaseN = static_cast<uint64_t>(AscendC::Te::Get<1>(params.l1TileShape));
        kL1Size_ = static_cast<uint64_t>(AscendC::Te::Get<2>(params.l1TileShape));
        scaleKL1Size_ = static_cast<uint64_t>(AscendC::Te::Get<3>(params.l1TileShape));
        kL0Size_ = static_cast<uint64_t>(AscendC::Te::Get<2>(params.l0TileShape));
    ...
    {
        auto layoutL0C =
            AscendC::Te::MakeFrameLayout<AscendC::Te::NZLayoutPtn, AscendC::Std::Int<BLOCK_CUBE>>(mL1Len_, nL1Len_);
        ...
        for (uint64_t kLoopIdx = 0; kLoopIdx < kTileCount_; ++kLoopIdx) {
            ...
            auto tensorBL1 = l1Storage_.MakeWeightTensor(
                l1BufIdx, 0, 0, Align16(nL1Len_), nL1Len_, kL1LenAligned);
            ...
            IterateMatmul(
                kLoopIdx, tensorL0C, tensorAL1, tensorBL1, tensorScaleAL1, tensorScaleBL1, tensorBiasL1, kL1Len,
                scaleWindowIter * kL1Size_, l1DataSlot, scaleASlot, scaleBSlot, l0cSlot);
    ...
    __aicore__ inline void InitBufferSlots(uint64_t baseN)
    {
        ...
        for (uint32_t index = 0; index < DOUBLE_BUFFER_COUNT; ++index) {
            ...
            bufferManager_.InitBL1(
                scaleASlot, l1Storage_.ScaleAOffset(index), BufferLayout::L1BDataBufferId(scaleASlot));
            bufferManager_.InitBL1(
                scaleBSlot, l1Storage_.ScaleBOffset(index), BufferLayout::L1BDataBufferId(scaleBSlot));
        }
    ...
    __aicore__ inline void ClearConvertedWeightL1Tail(const TensorBL1& tensorBL1, uint64_t kL1Len)
    {
        ...
        auto tailTensor = tensorBL1.Slice(
            AscendC::Te::MakeCoord(static_cast<int64_t>(kPhysicalLen), 0),
    ...
    __aicore__ inline void IterateMatmul(
        ...
        const TensorScaleAL1& tensorScaleAL1, const TensorScaleBL1& tensorScaleBL1, const TensorBiasL1& tensorBiasL1,
        uint64_t kL1Len, uint64_t kOffsetInScaleWindow, const BufferSlot& l1DataSlot,
        const BufferSlot& scaleASlot, const BufferSlot& scaleBSlot, const BufferSlot& l0cSlot)
    {
        ...
        for (uint64_t kL0Idx = 0; kL0Idx < kL0Iter; ++kL0Idx) {
            ...
            auto layoutAL0 =
                AscendC::Te::MakeFrameLayout<
                    AscendC::Te::NZLayoutPtn, AscendC::Std::Int<AscendC::Te::C0_ELEMENT<AType>>>(
            ...
            auto layoutBL0 =
                AscendC::Te::MakeFrameLayout<
                    AscendC::Te::ZNLayoutPtn, AscendC::Std::Int<AscendC::Te::C0_ELEMENT<AType>>>(
            ...
            bool needBias = hasBias_ && kLoopIdx == 0U && kL0Idx == 0U;
            {
                ...
                CopyScaleL1ToL0(
                    tensorScaleAL1, tensorScaleBL1, kOffsetInScaleWindow + kL0Offset, realL0K, l0Offset);
                if (needBias) {
    ...
    __aicore__ inline AscendC::Te::MmadParams MakeMmadParams(
        uint64_t kLoopIdx, uint64_t kL0Idx, uint64_t kL0Iter, uint64_t realL0K, bool needBias) const
    {
    ...
    __aicore__ inline void ComputeMmad(
        const AscendC::Te::MmadParams& mmadParams, const TensorL0C& tensorL0C, const TensorAL0& tensorAL0,
        const TensorBL0& tensorBL0, const TensorBt& tensorBt, bool needBias, const BufferSlot& l0Slot,
        const BufferSlot& l0cSlot, const BufferSlot& btSlot)
    {
        auto mmadAtom =
        ...
        uint64_t realL0K, uint64_t l0Offset)
    {
        ...
        uint64_t scaleKOffset =
            (kOffsetInScaleWindow / MXFP_DIVISOR_SIZE) * MXFP_MULTI_BASE_SIZE;
    ...
    __aicore__ inline void CopyCL0c2Gm(
        const TensorC& tensorC, const TensorL0C& tensorL0C, const BufferSlot& l0cSlot)
    {
    ...
    using BufferLayout =
        BufferIdLayout<QUADRUPLE_BUFFER_COUNT, QUADRUPLE_BUFFER_COUNT, DOUBLE_BUFFER_COUNT>;
