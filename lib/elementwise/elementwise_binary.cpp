    ...
    aclrtStream             stream)
{
    ...
    // ==========对标 hiptensor：Execute 时选择并缓存 solution ==========

    // 1. 检查是否已有缓存的 solution
    acltensor::ElementwiseBinarySolution* solution = nullptr;
    if (plan->pref->solution != nullptr) {
        solution = static_cast<acltensor::ElementwiseBinarySolution*>(plan->pref->solution);
    } else {
        // 2. 首次执行，从注册表查询解决方案
        auto& registry = acltensor::ElementwiseBinarySolutionRegistry::instance();
        ...
        // 3. 选择第一个并缓存到 pref->solution（对标 hiptensor）
        solution = solutions[0];
        plan->pref->solution = solution;
    }

    // 获取总元素数
    int64_t elemNum = static_cast<int64_t>(plan->opDesc->getTotalElements());
    if (elemNum <= 0)
    {
        return ACLTENSOR_STATUS_INVALID_VALUE;
    }
    ...
    // 执行内核
    return solution->executeBinary(alpha, A, gamma, C, D, elemNum, stream);
}
