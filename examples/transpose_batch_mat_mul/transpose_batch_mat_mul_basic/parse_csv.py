    ...
    with open(csv_path, "r", newline="") as f:
        ...
        for row in reader:
            cases.append({
                ...
                "trans_batch_a": row["trans_batch_a"].strip().lower() == "true",
...
def run_case(case):
    ...
    n = case["n"]
    batch = case["batch"]
    trans_a = str(case["trans_batch_a"]).lower()
    dtype = case["dtype"]
    ...
    gen_cmd = [
        ...
        str(m), str(k), str(n), str(batch),
        trans_a, dtype,
    ]
    if case["bias"] > 0:
        gen_cmd.append(str(case["bias"]))
    ...
    if not exec_path:
        exec_path = os.path.join(
            BUILD_DIR, "transpose_batch_mat_mul", "transpose_batch_mat_mul_basic",
    ...
    run_cmd = [
        exec_path, str(m), str(k), str(n), str(batch),
        trans_a, dtype, hf32, bias,
    ]
    ...
    verify_cmd = [
        ...
        str(m), str(batch), str(n),
        dtype,
    ]
    if case["bias"] > 0:
        verify_cmd.extend(["--bias", str(case["bias"])])
    if case.get("hf32"):
...
def main():
    ...
    for i, case in enumerate(cases, 1):
        ...
        print(f"  Shape: {case['m']} x {case['k']} x {case['n']}, batch={case['batch']}, "
              f"dtype={case['dtype']}, trans_batch_a={case['trans_batch_a']}")
