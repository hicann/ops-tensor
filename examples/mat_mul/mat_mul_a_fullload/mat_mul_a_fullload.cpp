...
/**
 * @file mat_mul_a_fullload.cpp
 * @brief Unified AFullLoad MatMul example supporting multiple dtypes and formats.
 *
 * A matrix is fully loaded into L1 buffer, B matrix is streamed in K-tiles.
 * Suitable for scenarios where M is small enough that A fits entirely in L1.
 *
...
struct TilingConfig {
    ...
    int64_t dtypeSize;
    static constexpr int L1_BUFFER_NUM = 4;
...
static TilingConfig GetTilingConfig(const std::string &dtype) {
    if (dtype == "float32") {
        ...
        return {256, 128, 128, 256, 128, 32, 2};
    }
}
...
static AFullLoadTiling ComputeTiling(TilingConfig &cfg, int64_t m, int64_t k, int64_t n) {
    ...
    cfg.nL1 = cfg.baseN;
    cfg.kL1 = cfg.baseK * TilingConfig::L1_BUFFER_NUM;
...
using BlockScheduler = Blaze::Gemm::Block::BlockSchedulerMatmulBasic<ProblemShape, Blaze::Gemm::A_FULL_LOAD_MODE>;
...
__global__ __aicore__ void matmul_a_fullload_kernel(GM_ADDR aGM, GM_ADDR bGM, GM_ADDR cGM, GM_ADDR biasGM, int64_t m,
    ...
    using DispatchPolicy = Blaze::Gemm::MatmulMultiBlockAFullLoad<>;
    ...
    Params params = {
        {m, n, k, 1},
        {aGM, bGM, cGM, biasGM, static_cast<uint64_t>(k), static_cast<uint64_t>(mL1), static_cast<uint64_t>(kL1),
         static_cast<uint32_t>(baseM), static_cast<uint32_t>(baseN), static_cast<uint32_t>(baseK),
         TilingConfig::L1_BUFFER_NUM, TilingConfig::L0C_DB},
        {},
        {static_cast<uint32_t>(mL1), static_cast<uint32_t>(baseN), static_cast<uint32_t>(kL1),
...
static void Run(const CliArgs &args) {
    ...
    AFullLoadTiling tiling = ComputeTiling(tilingCfg, args.m, args.k, args.n);
    ...
    std::cout << "  L0 Tile  : [" << tilingCfg.baseM << ", " << tilingCfg.baseN << ", " << tilingCfg.baseK << "]"
              << std::endl;
    std::cout << "  oriK     : " << args.k << " (A fully loaded)" << std::endl;
