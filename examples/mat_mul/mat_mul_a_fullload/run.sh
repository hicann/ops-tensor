...
EXAMPLE_M[mat_mul_a_fullload]=32
EXAMPLE_K[mat_mul_a_fullload]=512
EXAMPLE_N[mat_mul_a_fullload]=256
EXAMPLE_TA[mat_mul_a_fullload]=false
EXAMPLE_TB[mat_mul_a_fullload]=false
EXAMPLE_DTYPE[mat_mul_a_fullload]=float16
EXAMPLE_HF32[mat_mul_a_fullload]=false
EXAMPLE_BIAS[mat_mul_a_fullload]=0
EXAMPLE_FORMAT[mat_mul_a_fullload]="(ND,ND)"
...
Options:
  --target=<name>    Specify a single example to run (e.g. mat_mul_a_fullload).
...
preflight() {
    ...
    if [[ -z "${ASCEND_HOME_PATH:-}" ]]; then
        ...
        log_error "ASCEND_HOME_PATH directory does not exist: ${ASCEND_HOME_PATH}"; missing=1
    else log_info "ASCEND_HOME_PATH: ${ASCEND_HOME_PATH}"; fi
    if ! command -v bisheng &>/dev/null; then log_error "bisheng compiler not found"; missing=1
...
resolve_params() {
    local example="$1"
    if [[ -z "${EXAMPLE_M[$example]+x}" ]]; then log_error "No built-in defaults for: ${example}"; exit 1; fi
...
do_build() {
    local target="${EXAMPLE:-${TARGET:-mat_mul_a_fullload}}"
...
run_csv_cases() {
    local csv_file="$1"
    if [[ ! -f "$csv_file" ]]; then log_error "CSV file not found: $csv_file"; return 1; fi
    local target="${TARGET:-mat_mul_a_fullload}"
    ...
    if [[ $? -eq 0 ]]; then log_success "All test cases completed. Results: $result_file"; return 0
    else log_error "Some test cases failed."; return 1; fi
}
...
run_example() {
    ...
    if [[ "$BUILD_ONLY" == true ]]; then log_info "Build-only mode"; return 0; fi
    do_run
    local run_result=$?
...
main() {
    echo "=========================================="
    echo "  mat_mul_a_fullload One-Click Runner"
    ...
    for example in $EXAMPLES; do
        ...
        if [[ $rc -eq 0 ]]; then
            if [[ "$MODE" == "single" ]]; then echo ""; log_success "[PASS] ${example}"; fi
            results="${results}${example}: PASS\n"
        else
            if [[ "$MODE" == "single" ]]; then echo ""; log_error "[FAIL] ${example}"; fi
    ...
    if [[ "$MODE" == "multi" ]]; then
        ...
        if [[ "$all_passed" == true ]]; then log_success "All examples PASSED"
        else log_error "Some examples FAILED"; fi
    fi
    if [[ "$all_passed" != true ]]; then exit 1; fi
}
