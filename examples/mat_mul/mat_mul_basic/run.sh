...
EXAMPLE_M[mat_mul_basic]=128
EXAMPLE_K[mat_mul_basic]=512
EXAMPLE_N[mat_mul_basic]=128
EXAMPLE_TA[mat_mul_basic]=false
EXAMPLE_TB[mat_mul_basic]=false
EXAMPLE_DTYPE[mat_mul_basic]=float16
EXAMPLE_HF32[mat_mul_basic]=false
EXAMPLE_BIAS[mat_mul_basic]=0
EXAMPLE_FORMAT[mat_mul_basic]="(ND,ND)"
...
Options:
  --target=<name>    Specify a single example to run (e.g. mat_mul_basic).
...
do_build() {
    local target="${EXAMPLE:-${TARGET:-mat_mul_basic}}"
...
run_csv_cases() {
    ...
    local target="${TARGET:-mat_mul_basic}"
...
main() {
    echo "=========================================="
    echo "  mat_mul_basic One-Click Runner"
