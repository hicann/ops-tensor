...
/**
 * @file mat_mul_fixpipe_opti.cpp
 * @brief FixpipeOpti MatMul example: B full load + AIC/AIV split pipeline.
 *
 * B matrix is fully loaded into L1 buffer. AIC computes mmad while AIV handles
 * fixpipe copy-out, overlapping computation and data movement.
 * Suitable for large-M, small-N scenarios where B fits in L1.
 *
...
#define LAUNCH_KERNEL_IMPL()                                                                                         \
    matmul_fixpipe_kernel<A_TYPE, B_TYPE, C_TYPE, BIAS_TYPE, LAYOUT_A, LAYOUT_B, LAYOUT_C>                           \
...
static FixpipeTiling ComputeTiling(TilingConfig &cfg, int64_t m, int64_t k, int64_t n) {
...
__global__ __aicore__ void matmul_fixpipe_kernel(GM_ADDR aGM, GM_ADDR bGM, GM_ADDR cGM, GM_ADDR biasGM, int64_t m,
    ...
    using DispatchPolicy = Blaze::Gemm::MatmulMultiBlockFullLoadOrFixpipe<
        Blaze::Gemm::ND_ALIG_1V2_FIXPIPE, Blaze::Gemm::B_FULL_LOAD_MODE, 0, Blaze::Gemm::KernelMmadMultiBlockFixpipeOpti>;
    ...
    using BlockEpilogue = Blaze::Epilogue::Block::BlockEpilogueFixpipe<C_TYPE, C_TYPE, DispatchPolicy>;
    ...
    Params params = {
        ...
        {aGM, bGM, cGM, biasGM, nullptr, nullptr, static_cast<uint64_t>(k), static_cast<uint64_t>(mL1),
         ...
         static_cast<uint32_t>(baseN), static_cast<uint32_t>(baseK), TilingConfig::L1_BUFFER_NUM,
         TilingConfig::L0C_DB, 0, 1},
        {cGM},
...
static void Run(const CliArgs &args) {
    ...
    FixpipeTiling tiling = ComputeTiling(tilingCfg, args.m, args.k, args.n);
