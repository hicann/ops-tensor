...
#define LAUNCH_KERNEL_IMPL()                                                                                         \
    bmm_broadcast_kernel<A_TYPE, B_TYPE, C_TYPE, BIAS_TYPE, LAYOUT_A, LAYOUT_B>                            \
        <<<p.blockNum, 0, p.stream>>>(p.dA, p.dB, p.dC, p.dBias, p.m, p.n, p.k, p.batch, p.batchA, p.batchB,         \
                                       p.cfg->mL1, p.cfg->nL1, p.cfg->kL1, p.cfg->baseM, p.cfg->baseN, p.cfg->baseK, \
                                       p.tiling->mTailCnt, p.tiling->nTailCnt, p.isHf32)
...
static bool ParseCliArgs(int argc, const char **argv, CliArgs &args) {
    ...
    if (argc >= 8) args.transA = ParseBool(argv[7]);
    if (argc >= 9) args.transB = ParseBool(argv[8]);
    if (argc >= 10) args.dtype = argv[9];
    if (argc >= 11) args.isHf32 = ParseBool(argv[10]);
    if (argc >= 12) args.bias = std::atoll(argv[11]);
...
static void Run(const CliArgs &args) {
    ...
    LaunchParams launchParams = {deviceA,  deviceB,  deviceC,  deviceBias, args.m,     args.n,     args.k,
                                 args.batch, args.batchA, args.batchB, blockNum, &tiling,    &tilingCfg,
