...
def gen_batch_data(m, k, n, batch_a, batch_b, batch_c, transpose_a, transpose_b, dtype, bias_size=0):
    ...
    a_broadcast = a_cpu_t.expand(batch_c, -1, -1) if batch_a == 1 and batch_c > 1 else a_cpu_t
    b_broadcast = b_cpu_t.expand(batch_c, -1, -1) if batch_b == 1 and batch_c > 1 else b_cpu_t
...
if __name__ == "__main__":
    if len(sys.argv) < 10:
        print("Usage: python3 gen_data_batch.py m k n batch_a batch_b batch_c transA transB dtype [bias]")
        sys.exit(1)
    ...
    gen_batch_data(m, k, n, batch_a, batch_b, batch_c, transpose_a, transpose_b, DATA_TYPE, bias)
