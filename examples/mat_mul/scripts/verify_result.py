...
def _print_large_tensor_summary(golden_tensor, npu_output_tensor, m, n, dtype_str, ratio_tol):
    ...
    ratio_mask = _compute_ratio_error_mask(g, p, abs_err, no_non_finite, dtype_str, ratio_tol)
...
def verify_result(m, n, dtype_str, is_hf32=False):
    ...
    if numel <= FULL_TENSOR_PRINT_MAX_ELEMENTS:
        ...
        _print_large_tensor_summary(golden_tensor, npu_output_tensor, m, n, dtype_str, ratio_tol)
    ...
    if error_ratio > ratio_tol:
        ...
        print(f"top {min(max_display, all_error_count)} error points (sorted by abs_diff desc):")
        print(f"  {'(row,col)':<14s}  {'golden':>14s}  {'npu':>14s}  {'abs_diff':>14s}  {'rel_diff':>14s}")
        ...
        if all_error_count > max_display:
            print(f"  ... and {all_error_count - max_display} more error points (showing top {max_display} only)")
