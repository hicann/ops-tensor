...
def verify_batch_result(m, n, batch, dtype_str, is_hf32=False):
    ...
    if dtype_str == "float32":
        ...
        metric = torch.where(max_ab > 0, abs_diff / max_ab,
                             torch.where(abs_diff == 0, torch.zeros_like(abs_diff), torch.full_like(abs_diff, float("inf"))))
    else:
        metric = abs_diff
    ...
    print(f"ratio error count: {error_count}/{total_elements}, error ratio: {error_ratio:.6f}")

    return error_ratio <= ratio_tol


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Verify batch matmul NPU output against CPU golden.")
    ...
    try:
        res = verify_batch_result(args.m, args.n, args.batch, args.dtype, is_hf32=args.hf32)
        if not res:
