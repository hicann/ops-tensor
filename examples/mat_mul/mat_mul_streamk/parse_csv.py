...
def parse_csv(csv_path):
    """Read testcase.csv and return a list of case dicts."""
    cases = []
...
def run_case(case):
    ...
    status: "PASS" | "FAIL"
    stage:  "verify" | "kernel" | "gen_data"
    message: error detail or empty string on PASS.
    """
    m = case["m"]
    k = case["k"]
    n = case["n"]
    ...
    if not exec_path:
        exec_path = os.path.join(BUILD_DIR, "mat_mul", "mat_mul_streamk", "mat_mul_streamk")
...
def main():
    ...
    if len(sys.argv) >= 4:
        ...
        print("  No args: use default testcase.csv and auto-discover executable")
        sys.exit(1)
