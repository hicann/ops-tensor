...
declare -A EXAMPLE_M EXAMPLE_K EXAMPLE_N EXAMPLE_BATCH EXAMPLE_BA EXAMPLE_BB
declare -A EXAMPLE_IL1 EXAMPLE_IL0 EXAMPLE_TA EXAMPLE_TB EXAMPLE_DTYPE EXAMPLE_HF32 EXAMPLE_BIAS

EXAMPLE_M[mat_mul_iterbatch_broadcast]=32
EXAMPLE_K[mat_mul_iterbatch_broadcast]=64
EXAMPLE_N[mat_mul_iterbatch_broadcast]=32
EXAMPLE_BATCH[mat_mul_iterbatch_broadcast]=8
EXAMPLE_BA[mat_mul_iterbatch_broadcast]=1
EXAMPLE_BB[mat_mul_iterbatch_broadcast]=8
EXAMPLE_IL1[mat_mul_iterbatch_broadcast]=4
EXAMPLE_IL0[mat_mul_iterbatch_broadcast]=2
EXAMPLE_TA[mat_mul_iterbatch_broadcast]=false
EXAMPLE_TB[mat_mul_iterbatch_broadcast]=false
EXAMPLE_DTYPE[mat_mul_iterbatch_broadcast]=float16
EXAMPLE_HF32[mat_mul_iterbatch_broadcast]=false
EXAMPLE_BIAS[mat_mul_iterbatch_broadcast]=0

TARGET=""
CASE_FILE=""
SKIP_BUILD=false
BUILD_ONLY=false
M="" K="" N="" BATCH="" BA="" BB="" IL1="" IL0="" TRANS_A="" TRANS_B="" DTYPE="" IS_HF32="" BIAS=""
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        *)
            ...
            elif [[ -z "$BATCH" ]]; then BATCH="$1"
            elif [[ -z "$BA" ]]; then BA="$1"
            elif [[ -z "$BB" ]]; then BB="$1"
            elif [[ -z "$IL1" ]]; then IL1="$1"
            elif [[ -z "$IL0" ]]; then IL0="$1"
...
preflight() {
    ...
    if ! command -v bisheng &>/dev/null; then log_error "bisheng not found"; missing=1; fi
    if ! command -v g++ &>/dev/null; then log_error "g++ not found"; missing=1; fi
    if ! command -v python3 &>/dev/null; then log_error "python3 not found"; missing=1; fi
    if ! command -v cmake &>/dev/null; then log_error "cmake not found"; missing=1; fi
...
resolve_params() {
    ...
    BA_RESOLVED="${BA:-${EXAMPLE_BA[$example]}}"
    BB_RESOLVED="${BB:-${EXAMPLE_BB[$example]}}"
    IL1_RESOLVED="${IL1:-${EXAMPLE_IL1[$example]}}"
    IL0_RESOLVED="${IL0:-${EXAMPLE_IL0[$example]}}"
...
do_build() {
    local target="${EXAMPLE:-${TARGET:-mat_mul_iterbatch_broadcast}}"
...
do_gen_data() {
    ...
    (cd "${OP_SCRIPTS_DIR}" && python3 gen_data_batch.py \
        "${M_RESOLVED}" "${K_RESOLVED}" "${N_RESOLVED}" \
        "${BA_RESOLVED}" "${BB_RESOLVED}" "${BATCH_RESOLVED}" \
...
do_run() {
    ...
    if ! (cd "${OP_SCRIPTS_DIR}" && "${exec_path}" \
        "${M_RESOLVED}" "${K_RESOLVED}" "${N_RESOLVED}" \
        "${BATCH_RESOLVED}" "${BA_RESOLVED}" "${BB_RESOLVED}" \
        "${IL1_RESOLVED}" "${IL0_RESOLVED}" \
        "${TA_RESOLVED}" "${TB_RESOLVED}" "${DTYPE_RESOLVED}" \
...
do_verify() {
    ...
    if ! (cd "${OP_SCRIPTS_DIR}" && python3 "${OP_SCRIPTS_DIR}/verify_result_batch.py" \
        "${M_RESOLVED}" "${N_RESOLVED}" "${BATCH_RESOLVED}" "${DTYPE_RESOLVED}" ${hf32_flag}); then
...
run_csv_cases() {
    ...
    local target="${TARGET:-mat_mul_iterbatch_broadcast}"
...
run_example() {
    ...
    echo "  Batch:   C=${BATCH_RESOLVED}, A=${BA_RESOLVED}, B=${BB_RESOLVED}"
    echo "  IterBatch: L1=${IL1_RESOLVED}, L0=${IL0_RESOLVED}"
...
main() {
    echo "=========================================="
    echo "  mat_mul_iterbatch_broadcast One-Click Runner"
    echo "=========================================="
    preflight
    if [[ -n "$CASE_FILE" ]]; then
